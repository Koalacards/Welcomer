...
class SetupCommands(commands.Cog):
    ...
    async def set_welcome_channel(self, interaction: discord.Interaction, channel:discord.TextChannel):
        ...
        await send(interaction, embed=create_embed(title, description, colour), view=url_view, ephemeral=True)


    @app_commands.command(name='add-welcome-role')
    @app_commands.describe(role="Role to welcome the users with!")
    @app_commands.default_permissions(manage_guild=True)
    async def add_welcome_role(self, interaction: discord.Interaction, role:discord.Role):
    ...
    @app_commands.default_permissions(manage_guild=True)
    async def remove_welcome_role(self, interaction: discord.Interaction, role:discord.Role):
    ...
    @app_commands.describe(welcome_message="The message all users will get when they enter the server!")
    @app_commands.default_permissions(manage_guild=True)
    async def set_welcome_message(self, interaction: discord.Interaction, welcome_message:str):
